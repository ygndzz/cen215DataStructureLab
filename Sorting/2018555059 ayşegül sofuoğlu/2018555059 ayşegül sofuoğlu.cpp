...
int main()
{
	...
	int myNumber[] = { 2, 0, 1, 8, 5, 5, 5, 0, 5, 9 };
	int size_N = sizeof(myNumber) / sizeof(myNumber[0]);
	int num[size_N];
	clock_t timer;
	printf("My School Number is : ");
	print_N(&myNumber[0], size_N);
	printf("\n");
	
	// My Name Array
	char myName[] = "aysegul sofuoglu";
	int size_L = sizeof(myName) / sizeof(myName[0]);
	char let[17];
	printf("My Name is : ");
	print_L(&myName[0], size_L);
	...
	//for Number
	rest_N(&myNumber[0], &num[0], size_N);
	timer = clock();
