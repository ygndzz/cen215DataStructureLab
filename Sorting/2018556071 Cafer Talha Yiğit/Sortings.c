...
void QuickSort(int num[], int left, int right)
{
	if(left < right)
	{
		...
		while(1)
		{
			while(num[lo] < num[piv])
			{
				lo++;
			}	
			while(num[hi] > num[piv] && hi > left)
			{
				hi--;
			}
			if(lo < hi)
...
void QuickSortChar(char num[], int left, int right) {
	int i, j;
	i = left;
	j = right;
	...
	if (left < j) {
		QuickSortChar(num, left, j);
	}
	if (i < right) {
		QuickSortChar(num, i, right);
	}
}
...
void HeapChar(char arr[], int size, int i) {
	int left_child, right_child, max;
	max = i;
	left_child = i * 2 + 1;
	right_child = i * 2 + 2;
...
int main() {
	int i;
	clock_t timer;
	int NO1[10] = { 2,0,1,8,5,5,6,0,7,1};
	char Name1[] = "cafer talha yigit";
	int k = sizeof(Name1);
	printf("\t\t My number: ");
	for (i = 0; i < 10; i++) {
		printf("%d", NO1[i]);
	...
	timer = clock() - timer;
	PrintArray(NO1, 10);
	printf("\nTime_of_int : %f", (((double)timer) / CLOCKS_PER_SEC));
	printf("\n");
	timer = clock();
	InsertionsortChar(Name1, k);
	timer = clock() - timer;
	PrintChar(Name1, k);
	printf("\nTime_of_char: %f", (((double)timer) / CLOCKS_PER_SEC));
	printf("\n\n*****************************************\n\n");



	int NO2[10] = { 2,0,1,8,5,5,6,0,7,1 };
	char Name2[] = "cafer talha yigit";
	...
	int NO3[10] = { 2,0,1,8,5,5,6,0,7,1 };
	char Name3[] = "cafer talha yigit";
	...
	int NO4[10] = { 2,0,1,8,5,5,6,0,7,1 };
	char Name4[] = "cafer talha yigit";
	...
	int NO5[] = { 2,0,1,8,5,5,6,0,7,1 };
	char Name5[] = "cafer talha yigit";
	int size = sizeof(NO5)/sizeof(NO5[0]);
	printf("Sorting with Quick: \n");
	timer = clock();
	QuickSort(NO5, 0, size - 1);
	timer = clock() - timer;
	PrintArray(NO5, size);
	printf("\nTime_of_int: %f", (((double)timer) / CLOCKS_PER_SEC));
	printf("\n");
	timer = clock();
	QuickSortChar(Name5,0, k-1);
	timer = clock() - timer;
	PrintChar(Name5, k);
	printf("\nTime_of_char: %f", (((double)timer) / CLOCKS_PER_SEC));
	printf("\n\n*****************************************\n\n");
	...
	int NO6[10] = { 2, 0, 1, 8, 5, 5, 6, 0, 7, 1 };
	char Name6[] = "cafer talha yigit";
